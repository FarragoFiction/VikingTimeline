...
function createScenesForSession(session){
	session.scenes = [new LateWithStarBux(session), new WasteBetterThanJREverCould(session), new StartDemocracy(session), new JackBeginScheming(session), new KingPowerful(session), new QueenRejectRing(session), new GiveJackBullshitWeapon(session), new JackPromotion(session), new JackRampage(session)];
...
function printCorruptionMessage(msg, url, lineNo, columnNo, error){
	...
	if(curSessionGlobalVar.crashedFromPlayerActions){
		...
		recomendedAction = "TRY HOLDING 'SHIFT' AND CLICKING REFRESH TO CLEAR YOUR CACHE. IF THE BUG PERSISTS, CONTACT NO ONE AT ALL. YOU CANNOT CONVINCE THEM TO FIX SESSION: " + scratchedLineageText(curSessionGlobalVar.getLineage());
	}
	var message = [
            'Message: ' + msg,
            'URL: ' + url,
