...
function createScenesForSession(session){
	session.scenes = [new WasteBetterThanJREverCould(session), new StartDemocracy(session), new JackBeginScheming(session), new KingPowerful(session), new QueenRejectRing(session), new GiveJackBullshitWeapon(session), new JackPromotion(session), new JackRampage(session)];
